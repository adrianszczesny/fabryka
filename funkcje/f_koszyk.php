...
$conn = new mysqli($db_host, $db_user, $db_pass, $db_name);
    $cena = 0;
    $wynik = $conn->query("SELECT product.cena, sprzedaz.ilosc FROM sprzedaz INNER JOIN product ON product.id_produktu = sprzedaz.id_produktu WHERE id_zamowienia=$id_zamowienia AND id_uzytkownika=$id_usera");
    while($r = mysqli_fetch_array($wynik, MYSQLI_ASSOC))
    {
        $cena += $r['cena']*$r['ilosc'];
    }
