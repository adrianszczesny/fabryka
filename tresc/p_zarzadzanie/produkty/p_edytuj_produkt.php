...
$conn = new mysqli($db_host, $db_user, $db_pass, $db_name);
    ...
    $wynik_kurs_nazwa = $conn->query("SELECT * FROM `product` WHERE id_produktu={$id_produktu}");
    while ($r = mysqli_fetch_array($wynik_kurs_nazwa,MYSQLI_ASSOC)) 
    {
        $b_nazwa = $r['nazwa'];
        $b_cena = $r['cena'];
        $b_ilosc = $r['ilosc'];
    ...
    {   // sprawdzamy, czy pola są wypełnione
        ...
        {         
            ...
            if($conn->query("UPDATE product SET nazwa='{$_POST['nazwa']}', cena='{$_POST['cena']}', ilosc='{$_POST['ilosc']}', opis='{$_POST['opis']}' WHERE id_produktu={$id_produktu}")===TRUE)
            {   
